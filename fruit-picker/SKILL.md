...
- 需要读取或发送示意图时，先找本地 `references/anchors/<文件名>`；本地缺失则读 [references/anchors/README.md](references/anchors/README.md)，按其中的仓库地址获取同名文件。两处都不可用时不编造图中内容，按文字规则继续，并降低相关视觉档位的把握。
